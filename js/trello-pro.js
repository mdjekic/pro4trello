 ...
 * @return {string}
 */
let calculateTime = function (startTime, endTime) {
	let times = [0, 0];
 ...
 * @return {string}
 */
let calculatePrice = function (price1, price2) {
...
let refreshListsAndStats = function () {
	...
	for (let i = 0; i < $lists.length; i++) {
		...
		$this.find('.tpro-time-entry').each(function () {
			...
			list.totalTime = calculateTime(list.totalTime, t);
		});
		$this.find('.list-card').filter(visibleFilter).find('.tpro-time-entry').each(function () {
			let t = jQuery.trim(jQuery(this).text());
			list.totalVisibleTime = calculateTime(list.totalVisibleTime, t);
		});
		...
		$this.find('.tpro-price-entry').each(function () {
			let t = jQuery.trim(jQuery(this).text());
			list.totalPrice = calculatePrice(list.totalPrice, t);
		});
		$this.find('.list-card').filter(visibleFilter).find('.tpro-price-entry').each(function () {
			let t = jQuery.trim(jQuery(this).text());
			list.totalVisiblePrice = calculatePrice(list.totalVisiblePrice, t);
		});
...
let buildHashtagsFilter = function () {
	...
	$menuItem.append(jQuery('<span class="board-header-btn-text"><span>Hash: </span></span>').append($filter));
...
/**
 * Builds the Orientation Filter
 */
let buildListOrientationToggle = function() {
	log('building list orientation toggle...');

	// create menu item
	let $menuItem = jQuery('<a id="tpro-header-button-list-orientation" class="board-header-btn" href="#"></a>');
	if (TrelloPro.settings['horizontal-lanes']) {
		$menuItem.append('<span class="board-header-btn-icon icon-sm"><i class="fa fa-arrow-down"></i></span>');
		$menuItem.data('horizontal', true);
	} else {
		$menuItem.append('<span class="board-header-btn-icon icon-sm"><i class="fa fa-arrow-right"></i></span>');
		$menuItem.data('horizontal', false);
	}	
	...
	$menuItem.on('click', function (e) {
		...
		if ($menuItem.data('horizontal')) {
			$this.find('i').addClass('fa-arrow-right');
			$this.find('i').removeClass('fa-arrow-down');
			$menuItem.data('horizontal', false);
			TrelloPro.settings['horizontal-lanes'] = false;
		} else {
			$this.find('i').removeClass('fa-arrow-right');
			$this.find('i').addClass('fa-arrow-down');
			$menuItem.data('horizontal', true);
			TrelloPro.settings['horizontal-lanes'] = true;
		}

		toggleCssInject('horizontal-lanes');
		saveSettings();
