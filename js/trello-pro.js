...
let processCardTitleChange = function ($title, refreshData) {
	return new Promise((resolve, reject) => {
		...
		if (jQuery.trim($title.text()).length < 5) {
			resolve();
			return;
		}
		if (html.indexOf('<span class="tpro"></span>') > -1) {
			resolve();
			return;
		}
...
let rebuildDynamicStyles = function () {
	...
	if (TrelloPro.loaded) {
		...
		if (TrelloPro.settings['parse-projects'] && TrelloPro.settings.filters.project) {
			...
			if (TrelloPro.data.projects.map(function (project) {
					return project.key;
				}).indexOf(TrelloPro.settings.filters.project) > -1) {
		...
		if (TrelloPro.settings['parse-labels'] && TrelloPro.settings.filters.label) {
			...
			if (TrelloPro.data.labels.map(function (label) {
					return label.key;
				}).indexOf(TrelloPro.settings.filters.label) > -1) {
		...
		if (TrelloPro.settings['parse-hashtags'] && TrelloPro.settings.filters.hashtag) {
			...
			if (TrelloPro.data.hashtags.map(function (hashtag) {
					return hashtag.key;
				}).indexOf(TrelloPro.settings.filters.hashtag) > -1) {
...
let buildData = function () {
	...
	let priorities = [{
			key: 'no',
			value: 'No Priority',
			cardCount: jQuery('.tpro-priority-no').length
		},
		{
			key: 'low',
			value: 'Low Priority',
			cardCount: jQuery('.tpro-priority-low').length
		},
		{
			key: 'medium',
			value: 'Medium Priority',
			cardCount: jQuery('.tpro-priority-medium').length
		},
		{
			key: 'high',
			value: 'High Priority',
			cardCount: jQuery('.tpro-priority-high').length
		}
	];
